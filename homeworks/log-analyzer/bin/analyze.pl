...
sub parse_file {
    ...
    my $date_ptrn = '\[(?<date>(?<day>\d{2})\/(?<mounth>\w{3})\/(?<year>\d{4}):(?<hours>\d{2}):(?<minutes>\d{2})):(?<seconds>\d{2}) (?<timezone>\+\d{4})\]';
...
sub report {
    ...
    foreach my $ip (@$list) {
        ...
        printf $row_format, $ip, $result->{$ip}->{count},
          $result->{$ip}->{average}, $result->{$ip}->{data} / 1024, @str;
    }
}
